...
func initRouter(r *gin.Engine) {
	...
	apiRouter.POST("/publish/action/", middleware.VerifyUserLogin(), controller.Publish) //必写，发布视频调用该接口
	apiRouter.GET("/publish/list/", controller.PublishList)                              //必写，个人页面显示所有作品

	// extra apis - I
	apiRouter.POST("/favorite/action/", middleware.VerifyUserLogin(), controller.FavoriteAction) //必写，点赞调用该接口
	apiRouter.GET("/favorite/list/", controller.FavoriteList)                                    //必写，个人页面显示所有喜欢的作品
	apiRouter.POST("/comment/action/", middleware.VerifyUserLogin(), controller.CommentAction)   //必写，提交评论调用该接口
